...
vector<double> get_leaving_rates(const vector<double>& u, int m, int n, const vector<double>& G, const vector<double>& pi_c, const vector<double>& pi_d, double lambda, double w_i, double w_g, double mutation) {
    vector<double> L;
    L.reserve(n + 1);
    ...
    for (int i = 1; i < n; i++) {
        ...
        double coop_def_sum = m * u[i] * (n - i) * i * inv_n * (mutation * (1 + w_i * pi_d[i]) + (1 - mutation) * (1 + w_i * pi_c[i]));
...
vector<double> get_incoming_rates(const vector<double>& u, int m, int n, const vector<double>& G_payoff, const vector<double>& pi_c, const vector<double>& pi_d, double lambda, double I1, double w_i, double w_g, double mutation) {
    vector<double> G(n + 1, 0.0);
    ...
    if (I1 > 0) {
        G[I1 - 1] = (n - I1) * (1 + w_i * pi_d[I1]) * I1_inv_n * mutation + u[I1 - 1] * lambda * (1 + w_g * G_payoff[I1 - 1]);
    }

    if (I1 < n) {
        G[I1 + 1] = I1 * one_minus_I1_inv_n * (1 + w_i * pi_c[I1]) * (1 - mutation) + u[I1 + 1] * lambda * (1 + w_g * G_payoff[I1 + 1]);
    }
...
int main(int argc, char** argv) {
    ...
    double mutation = 0.25; // probability of mutation
    ...
    *output_stream << m << " " << n << " " << lambda << " " << w_i << " " << w_g << endl;
    ...
    while (u[0] < m && u[n] < m) {
        ...
        vector<double> L = get_leaving_rates(u, m, n, G_payoff, pi_c, pi_d, lambda, w_i, w_g, mutation);
        ...
        vector<double> G = get_incoming_rates(u, m, n, G_payoff, pi_c, pi_d, lambda, I1, w_i, w_g, mutation);
